...
import { TrendingUp, TrendingDown, Clock, Plus, Search, Sparkles, X, CheckCircle } from 'lucide-react';
...
export function MarketCard({
    ...
    // Handle card click - if in cluster mode, toggle cluster selection
    const handleCardClick = (e: React.MouseEvent) => {
        if (showAddButton && onAddToCluster) {
            e.preventDefault();
            e.stopPropagation();
        ...
        <>
            ...
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                    ...
                    {isInCluster && (
                        <CheckCircle size={16} className="text-bullish" />
                    )}
                </div>
            ...
            <div className="flex gap-3 mb-4">
                {market.imageUrl && (
                    <div className="flex-shrink-0 mt-0.5">
                        <img
                            src={market.imageUrl}
                            alt="Market"
                            className="w-10 h-10 rounded-full object-cover bg-surface-2 border border-border"
                            onError={(e) => {
                ...
                <h3 className="text-text-primary font-medium text-sm leading-snug line-clamp-3 group-hover:text-bullish transition-colors">
                    {market.question}
                </h3>
            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <span
                                className={`text-sm font-medium ${changeInfo.isPositive
                                    ? 'text-success'
                                    : changeInfo.isNeutral
                                        ? 'text-text-secondary'
                                        : 'text-bearish'
    ...
    return (
        ...
        >
            {showAddButton ? (
                // In cluster mode: make entire card clickable to add/remove
                <div
                    onClick={handleCardClick}
                    className={`group bg-surface border rounded-xl p-4 transition-all duration-200 cursor-pointer ${isInCluster
                        ? 'border-bullish border-2 bg-bullish/5 shadow-md shadow-bullish/20'
                        : 'border-border hover:border-text-secondary hover:shadow-md'
                        }`}
                >
                    <CardContent />
                ...
                // Normal mode: link to market detail
