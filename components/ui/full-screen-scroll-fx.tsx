...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
    (
        ...
        useEffect(() => {
            // Initialize wordRefs array size
            if (wordRefs.current.length === 0) {
                wordRefs.current = sections.map(() => []);
            }
        }, [sections]);
        ...
        // Robust word capture
        const setWordRef = (sIdx: number, wIdx: number, el: HTMLSpanElement | null) => {
        ...
        const renderTitle = (title: string | ReactNode, sIdx: number) => {
            if (typeof title !== "string") return title;
            const words = title.split(/\s+/).filter(Boolean);
            return words.map((w, i) => (
                <span className="fx-word-mask" key={i}>
                    <span className="fx-word" ref={(el) => setWordRef(sIdx, i, el)}>{w}</span>
        ...
        useLayoutEffect(() => {
            ...
            // initial center words visibility
        ...
        return (
          ...
          .fx-header {
            grid-column: 1 / 13; align-self: start; 
            padding-top: 8vh;
            font-size: clamp(1.5rem, 6vw, 5rem); line-height: 1; text-align: center; color: var(--fx-text);
            z-index: 3;
          }
          ...
          .fx-content {
            ...
            align-items: center;
            height: 100%;
            padding: 8vh var(--fx-grid-px) 12vh;
          }
          ...
          .fx-track { will-change: transform; transition: transform 0.1s linear; }
          ...
          .fx-left-item.active { transform: translateX(10px); }
          .fx-right-item.active { transform: translateX(-10px); }

          .fx-center {
            display: grid; place-items: center; text-align: center; height: 60vh; overflow: visible; position: relative;
          }
          ...
          .fx-footer {
            grid-column: 1 / 13; align-self: end; padding-bottom: 8vh; text-align: center; z-index: 3;
          }
          ...
          @media (max-width: 1100px) {
            .fx-header { padding-top: 10vh; font-size: 2.5rem; }
            .fx-content {
            ...
            .fx-featured-title { font-size: 2.2rem; }
            .fx-footer { padding-bottom: 8vh; }
            .fx-footer-title { font-size: 1.6rem; }
          }

          @media (max-width: 600px) {
            .fx-header { font-size: 1.8rem; padding-top: 12vh; }
            .fx-featured-title { font-size: 1.6rem; }
            .fx-footer-title { font-size: 1.2rem; }
            .fx-progress { width: 160px; }
          }
        `}</style>
            </div>
        );
